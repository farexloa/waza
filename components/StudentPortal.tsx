...
export const StudentPortal: React.FC<StudentPortalProps> = ({ 
  ...
  // --- 1. MODAL: SOLICITUD PENDIENTE ---
  // (Este es el único bloqueo. Si se aprueba, desaparece y muestra el dashboard normal)
  ...
  // --- 2. DASHBOARD PRINCIPAL (Para estado NORMAL o APPROVED) ---
      ...
      <main className="flex-1 flex flex-col min-w-0 h-full relative">
        ...
        <header className="bg-white border-b border-gray-200 h-16 lg:h-20 px-4 lg:px-8 flex items-center justify-between z-10 flex-shrink-0">
           ...
           <div className="flex items-center gap-3">
              {/* STATUS DE SALIDA APROBADA (Visible en el header si está aprobado) */}
              {student.pickupAuthorization === PickupAuthStatus.APPROVED && (
                 <span className="flex items-center gap-1.5 px-3 py-1.5 bg-green-500 text-white rounded-full text-xs font-bold shadow-md shadow-green-200 animate-in fade-in">
                    <Icons.Check className="w-4 h-4" /> SALIDA APROBADA
                 </span>
              )}
